...
const register = (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE email = ?", [email], (error, results) => {
    ...
    db.query("INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?)", [id, name, email, hashedPassword], (error, results) => {
      if (error) {
        res.status(500).json({
          error: true,
...
const forgotPassword = (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE email = ?", [email], (error, results) => {
    ...
    const id = nanoid(16);
    db.query("INSERT INTO users_otp VALUES (?, ?, ?, NOW())", [id, email, generatedOTP], (error, results) => {
      if (error) {
        res.status(500).json({
          error: true,
...
const resetPassword = (req, res) => {
  ...
  db.query("SELECT * FROM users_otp WHERE email = ? AND otp = ? AND (TIMESTAMPDIFF(MINUTE, created_at, NOW())<5)", [email, otp], (error, results) => {
    if (error) {
      res.status(500).json({
        error: true,
